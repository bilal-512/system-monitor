...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    // Default update interval from config or 2000 ms
...
void MainWindow::setupProcessTab() {
    ...
    m_processSearch->setStyleSheet("color: black; background-color: white;");
    layout->addWidget(m_processSearch);

    m_processTable = new QTableWidget();
    m_processTable->setColumnCount(5);
    m_processTable->setHorizontalHeaderLabels({"PID", "Name", "Memory", "CPU %", "Threads"});
    ...
    m_processTable->sortByColumn(3, Qt::DescendingOrder); // Default sort by CPU Usage Descending

    layout->addWidget(m_processTable);

    QPushButton* btnSortCpu = new QPushButton("Sort by CPU");
    btnSortCpu->setStyleSheet("background-color: #27ae60; color: white; padding: 8px; font-weight: bold; border-radius: 4px;");
    ...
    btnSortMem->setStyleSheet("background-color: #2980b9; color: white; padding: 8px; font-weight: bold; border-radius: 4px;");
    ...
    btnDetails->setStyleSheet("background-color: #3498db; color: white; padding: 8px; font-weight: bold; border-radius: 4px;");
    connect(btnDetails, &QPushButton::clicked, this, &MainWindow::onViewDetailsClicked);

    QPushButton* btnKill = new QPushButton("End Task");
    btnKill->setStyleSheet("background-color: #c0392b; color: white; padding: 8px; font-weight: bold; border-radius: 4px;");
...
void MainWindow::onCpuUpdated(double usage) {
    ...
    m_cpuDetailsLabel->setText(QString("Load: %1% | Active Cores: %2")
                               .arg(usage, 0, 'f', 1)
                               .arg(sysmon::SystemInfo::getLogicalCoreCount())); // Approx
...
void MainWindow::onProcessesUpdated(const std::vector<sysmon::ProcessInfo>& procs) {
    ...
    int row = 0;
    m_processTable->setRowCount(static_cast<int>(procs.size())); // overestimate
    ...
    for (int i = 0; i < procs.size(); ++i) {
        ...
        m_processTable->setItem(row, 2, memItem);
        m_processTable->setItem(row, 3, cpuItem);
        m_processTable->setItem(row, 4, threadItem);
...
void MainWindow::onSortByMemoryClicked() {
    m_processTable->sortByColumn(2, Qt::DescendingOrder);
}
...
void MainWindow::onPerformanceStatsUpdated(const sysmon::PerformanceStats& stats) {
    ...
    // Lazy sync UI with config if empty
